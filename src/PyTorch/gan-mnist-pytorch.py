...
parser.add_argument('--ratio', type=float, default=1.0, help='Ratio of fake images/real images provided to the discriminator')
...
for epoch in range(1, args.epochs+1):
	for i, (images, _) in enumerate(data_loader):
		...
		real_output = Variable(Tensor(round(images.size(0) * max(1, ratio)), 1).fill_(1.0), requires_grad=False)
		fake_output = Variable(Tensor(round(images.size(0) * ratio), 1).fill_(0.0), requires_grad=False)
		# Training Generator
		optimizer_generator.zero_grad()
		z = Variable(Tensor(np.random.normal(0, 1, (round(images.shape[0]*max(1,ratio)), args.latent_dim))))
		generated_images = generator(z)
		lol = discriminator(generated_images)
		generator_loss = adversarial_loss(lol, real_output)
		generator_loss.backward()
		optimizer_generator.step()

		# z = Variable(Tensor(np.random.normal(0, 1, (round(images.shape[0]*1.5), args.latent_dim))))
		# generated_images = generator(z)
		# Training Discriminator
		optimizer_discriminator.zero_grad()
		discriminator_loss_real = adversarial_loss(discriminator(real_images), real_output.narrow_copy(0, 0, images.size(0)))
		discriminator_loss_fake = adversarial_loss(discriminator(generated_images.narrow_copy(0,0, round(images.size(0)*ratio)).detach()), fake_output)
	...
	genlosses.append(generator_loss.item())
	disclosses.append(discriminator_loss.item())
	bothlosses.append((generator_loss.item(), discriminator_loss.item()))
...
bothlosses = np.array(bothlosses)
genlosses = np.array(genlosses)
disclosses = np.array(disclosses)
print(genlosses)
plt.plot(bothlosses.T[0], label='Generator')
plt.plot(bothlosses.T[1], label='Discriminator')
...
plt.ylabel("Loss")
plt.legend()
plt.savefig(f'{args.output_dir}/gen_loss_plot.png')
plt.clf()
...
plt.ylabel("Loss")
plt.legend()
plt.savefig(f'{args.output_dir}/disc_loss_plot.png')
